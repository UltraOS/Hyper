...
{
    size_t blocks = (br->file_off & br->fs_block_mask) + br->bytes;
    blocks = ALIGN_UP(blocks, br->fs_block_mask + 1);

    return blocks >> file_block_shift(br->f);
}

bool do_bulk_read(struct bulk_read_req *br, file_get_range_t get_range)
{
